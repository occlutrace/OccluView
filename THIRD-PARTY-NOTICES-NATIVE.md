# Third-party notices — native components
...
* **The upstream reference is a tag, not a commit.** `manifold-csg-sys` checks
  out `v3.5.1`, and a tag can be moved. Two builds of the same OccluView commit
  can therefore link different C++ sources, and a retagged upstream would reach
  a signed installer. Setting `MANIFOLD_CSG_LIB_DIR` to a prebuilt library skips
  the clone entirely and is the way to make a build independent of it.
* **`cargo deny` and the CycloneDX SBOM do not see any of this.** Both walk the
  Cargo graph. License and advisory scanning therefore stops at the Rust
  wrappers, and the SBOM shipped with a release describes the Rust half of the
  binary. That is a real gap, stated here rather than left to be found.

---

## Manifold

Copyright the Manifold authors. Licensed under the Apache License, Version 2.0.
...
Copyright Intel Corporation and the oneTBB contributors. Licensed under the
...
Copyright Angus Johnson. Licensed under the Boost Software License 1.0.

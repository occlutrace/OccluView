...
/// Every directed edge of a mesh, sorted, with the triangle that owns it.
///
/// This was a `HashSet` of all directed edges beside a `HashMap` from the same
/// keys to their owners: two hashed inserts per edge, then one hashed lookup
/// per edge to find the twin. That is three million hashed operations on a
/// 980k-triangle arch, and it was the largest fixed cost in Close Holes --
/// measured end to end through the CLI on such an arch, 3.85 s against 2.95 s,
/// with byte-identical output.
///
/// Vertex indices fit in `u32` because `Mesh::new` refuses anything larger, so
/// a directed edge packs into one `u64` key and both lookups are a binary
/// search.

...
/// surface is mostly seated and a fifth of it is far away — the shape of a fit
/// that slid onto a neighbouring surface while most of the sampled patch
/// stayed put. `p95_abs` measures that tail, and this bound keeps it finite
/// and within reach.
///
/// A fifth of the matched surface sitting a full correspondence radius away is
/// not an alignment. The bound is not tighter, because the median limit
/// already carries the fine discrimination and the corpus numbers are the
/// calibration: a correct seating reports a median near the discretisation
/// error but a `p95_abs` that follows the surface's own noise, and a bound
/// tighter than the radius would risk refusing a correct seating on a coarse
/// scan. What it removes is the unbounded case: a pose whose discarded fifth is
/// arbitrarily far away is not authorized on the strength of its median alone.
...
pub struct IcpReport {
    ...
    /// read it. Trimmed residual statistics alone cannot certify a pose: a fit
    /// that seats nothing can still report a small median, because the trimmed
    /// set never contains the region that is far away.
...
impl IcpReport {
    ...
    /// the solver itself ranks poses by.
    ///
    ...
    /// and a wrong pose whose median of 0.190 sits just under a 0.2 mm median
    /// limit manages 0.072. The floor sits between them.
    ///
    /// A pose reported by a caller that does not compute seating leaves the field
    /// zero, and zero is refused: "this report never measured seating" must not
    /// read as "this pose seats nothing but is otherwise fine" — the gate is
    /// fail-closed.
    #[must_use]

...
/// Paint the deviation legend: the colour ramp with its numeric bounds and the
/// tolerance band marked, so a colour on screen can always be read as a number.
pub(crate) fn paint_legend(ui: &mut egui::Ui, settings: AlignSettings) {
    ...
    for step in 0..STEPS {
        ...
        let position = (step as f64 / (STEPS - 1) as f64).mul_add(2.0, -1.0);
        let color = occluview_align::ramp_color(
            position * settings.scale_mm,
    ...
    ui.horizontal(|ui| {
        ...
        label(ui, format!("−{:.2} mm", settings.scale_mm));

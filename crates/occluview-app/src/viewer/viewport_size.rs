...
/// The render target for a viewport that size: the viewport's own shape,
/// scaled to sit inside the size bounds.
///
/// One scale factor for both axes, and that is the whole point. Clamping each
/// axis on its own changes the target's shape, and the target is painted to
/// fill the viewport rect whatever shape it has — so the picture arrives
/// stretched. A 4K viewport is where this stopped being theoretical: 3840x2160
/// clamped per axis to 2560x2160, because only the width was over the bound,
/// and every model on screen came out half again too wide. The report named
/// fullscreen because that is where a 4K width first passes 2560; a
/// maximized window is over it just the same.
#[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]

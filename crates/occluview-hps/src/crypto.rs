...
mod tests {
    ...
    /// The three-way EKID matrix, which decides what key a package is actually
    /// opened with.
    ///
    /// Getting any branch wrong does not fail loudly: the wrong key produces
    /// plausible bytes that then fail the adler32 check, so every affected
    /// customer sees "integrity failure" on their own scans while the test
    /// suite stays green. Only the derivation itself can catch it.
    #[test]
    fn ekid_selects_which_key_a_locked_package_is_opened_with() {
        ...
        // EKID="1": the lock hash is APPENDED to the base key.
        ...
        // Any other EKID: the base key is used unchanged.

...
fn combined_com_source() -> String {
    [
        ...
        include_str!("com/preview/lifecycle.rs"),
    ]
    .join("\n")
}
...
fn the_preview_window_and_the_com_object_die_together() {
    ...
    let lifecycle = include_str!("com/preview/lifecycle.rs");
    let window = include_str!("com/preview/window.rs");

    assert!(
        preview.contains("mod lifecycle;"),
        "preview lifecycle must stay in its own module rather than grow the COM facade"
    );

//! What the gate and the pool do when a request does not end normally.
//!
//! Poisoned locks, a device creation that panics, a render that panics, and a
//! renderer that never comes back: each of these used to leave shared state a
//! later request could not recover from, and each is reproduced here.

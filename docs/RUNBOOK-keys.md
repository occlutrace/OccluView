...
Three secrets stand between this project and its users. Two of them have no
recovery path that the product controls, which is why this document exists: the
procedures below are cheap to write down now and impossible to improvise during
an incident.
...
| `OCCLUVIEW_SIGN_PFX_BASE64` (+ `OCCLUVIEW_SIGN_PFX_PASSWORD`) or `OCCLUVIEW_SIGN_CERT_SHA1` | Certificate authority's issuance; GitHub Actions secret | Authenticode on the MSI, the viewer and the shell DLL | Tagged releases fail their signature gate until a new certificate is issued | Revoke through the issuing CA; Authenticode timestamps limit the blast radius |
...
Authenticode signatures are timestamped, so already-shipped artifacts stay valid
after the certificate expires. Only new releases fail, at the signature gate in
the packaging job, which is deliberate: a tagged release that cannot be signed
must not publish. Renew before the expiry date rather than after a red release.
